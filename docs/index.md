...
クラウド型入居者管理システム「パレット管理」等の開発・運営。

### プロジェクト規模
3〜6 人チームでの開発

### 使用技術
TypeScript, NestJS, Ruby on Rails, Vue.js, Nuxt.js, PostgreSQL, AWS, Docker

### 役割
プロジェクトリーダー / 開発（要件定義、設計、コーディング、レビュー）
...
#### パレット管理 LINEミニアプリを活用した新規プロダクトの立ち上げ
- 構想からリリースまで3ヶ月で完遂
- 一から作り直すのではなく、今ある機能および財産を活用し「いかに作らないか」を徹底
- 車内で過去最速でのローンチを実現し、年間個人優秀賞・プロジェクト優秀賞をW受賞
- エンジニア、デザイナー、CS、QAを含むチームのチームリーダーとして、要件定義、開発ディレクション、デザインレビュー、進捗管理などプロジェクトを前進させるために必要な業務を対応。
- 入居時チェック機能や解約申請機能などの主要機能を実装し、管理会社の業務効率化に貢献。現在、複数社で本番稼働中
- クライアントからの問い合わせを3時間以内に解決することを達成指標としており90%以上達成

#### 汎用申請管理モジュールの新規構築
- 4名プロジェクトのプロジェクトリーダーとして、要求仕様定義、要件定義、技術選定、詳細設計、実装、進捗管理などプロジェクトを前進させるために必要な業務を遂行
- イミュータブルデータモデルによる申請データや添付ファイル、コメントの履歴管理を実装
- NestJSを採用し、型安全かつ保守性の高いAPI基盤を構築
- 申請業務（フォーム）に特化したマイクロサービスとして設計
- PostgreSQLのRLS（Row Level Security）を活用し、DBレベルでテナント間のデータ分離を保証するセキュアなマルチテナント構造を設計・実装
- フォーム定義やバリデーションロジックを共通化し、新フォーム追加時の開発コストを大幅に削減できる基盤を構築
- CI/CDの構築
